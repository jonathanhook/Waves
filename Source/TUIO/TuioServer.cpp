...
void TuioServer::sendFullMessages() {
	...
	(*fullPacket) << osc::BeginMessage( "/2Dcur") << "alive";
	...
	for (std::list<TuioCursor*>::iterator tuioCursor = cursorList.begin(); tuioCursor!=cursorList.end(); tuioCursor++) {
		...
		if ((fullPacket->Capacity()-fullPacket->Size())<CUR_MESSAGE_SIZE) {
			...
			(*fullPacket) << osc::BeginMessage( "/2Dcur") << "fseq" << -1 << osc::EndMessage;
		...
		(*fullPacket) << osc::BeginMessage( "/2Dcur") << "set";
	...
	(*fullPacket) << osc::BeginMessage( "/2Dobj") << "alive";
	...
	for (std::list<TuioObject*>::iterator tuioObject = objectList.begin(); tuioObject!=objectList.end(); tuioObject++) {
		...
		if ((fullPacket->Capacity()-fullPacket->Size())<OBJ_MESSAGE_SIZE) {
			...
			(*fullPacket) << osc::BeginMessage( "/2Dobj") << "fseq" << -1 << osc::EndMessage;
			(*fullPacket) << osc::EndBundle;
			socket->Send( fullPacket->Data(), fullPacket->Size() );
		...
		(*fullPacket) << osc::BeginMessage( "/2Dobj") << "set";
...
void TuioServer::sendEmptyCursorBundle() {
	...
	(*oscPacket) << osc::BeginMessage( "/2Dcur") << "alive" << osc::EndMessage;	
	(*oscPacket) << osc::BeginMessage( "/2Dcur") << "fseq" << -1 << osc::EndMessage;
...
void TuioServer::startCursorBundle() {	
	...
	(*oscPacket) << osc::BeginMessage( "/2Dcur") << "alive";
	for (std::list<TuioCursor*>::iterator tuioCursor = cursorList.begin(); tuioCursor!=cursorList.end(); tuioCursor++) {
...
void TuioServer::addCursorMessage(TuioCursor *tcur) {
	 ...
	 (*oscPacket) << osc::BeginMessage( "/2Dcur") << "set";
...
void TuioServer::sendCursorBundle(long fseq) {
	(*oscPacket) << osc::BeginMessage( "/2Dcur") << "fseq" << (int32)fseq << osc::EndMessage;
	(*oscPacket) << osc::EndBundle;
	socket->Send( oscPacket->Data(), oscPacket->Size() );
}
...
void TuioServer::sendEmptyObjectBundle() {
	oscPacket->Clear();	
	(*oscPacket) << osc::BeginBundleImmediate;
	(*oscPacket) << osc::BeginMessage( "/2Dobj") << "alive" << osc::EndMessage;	
	(*oscPacket) << osc::BeginMessage( "/2Dobj") << "fseq" << -1 << osc::EndMessage;
	(*oscPacket) << osc::EndBundle;
	socket->Send( oscPacket->Data(), oscPacket->Size() );
}
...
void TuioServer::startObjectBundle() {
	...
	(*oscPacket) << osc::BeginMessage( "/2Dobj") << "alive";
	for (std::list<TuioObject*>::iterator tuioObject = objectList.begin(); tuioObject!=objectList.end(); tuioObject++) {
...
void TuioServer::addObjectMessage(TuioObject *tobj) {
	(*oscPacket) << osc::BeginMessage( "/2Dobj") << "set";
...
void TuioServer::sendObjectBundle(long fseq) {
	(*oscPacket) << osc::BeginMessage( "/2Dobj") << "fseq" << (int32)fseq << osc::EndMessage;
	(*oscPacket) << osc::EndBundle;
	socket->Send( oscPacket->Data(), oscPacket->Size() );
}
